...
export default function GenericResourcePage({
  ...
  const header = (
    <div className="flex flex-col gap-4 border-b border-black/10 pb-5 sm:flex-row sm:items-center sm:justify-between dark:border-white/10">
      <div>
      ...
      {shouldShowCreateButton && (
        <div className="flex items-center gap-3">
          {title.toLowerCase() === 'departments' && (
            <button
              type="button"
              className="flex w-full items-center justify-center gap-2 rounded-xl bg-purple-600 px-4 py-2.5 text-sm font-semibold text-white shadow-sm shadow-purple-200 transition hover:bg-purple-700 sm:w-auto dark:shadow-black/30 dark:hover:bg-purple-500"
  ...
  return (
    <div className="min-h-screen space-y-6 bg-gradient-to-b from-purple-50 via-[#F8F6FC] to-purple-50 p-4 sm:p-6 dark:from-gray-950 dark:via-gray-900 dark:to-gray-950">
      ...
      {}
      <div className="hidden overflow-hidden rounded-2xl border border-purple-100 bg-white shadow-sm shadow-purple-100/50 sm:block dark:border-gray-800 dark:bg-gray-900 dark:shadow-black/30">
            ...
            <thead className="bg-purple-50 text-left text-gray-900 dark:bg-gray-800/60 dark:text-gray-100">
              <tr>
                {columns.map((column) => (
                  <th
                    key={column}
                    className="whitespace-nowrap px-4 py-3 text-xs font-semibold uppercase tracking-[0.15em]"
                  >
                ...
                {editPath && isHR && (
                  <th className="whitespace-nowrap px-4 py-3 text-center text-xs font-semibold uppercase tracking-[0.15em]">
                    Actions
                  </th>
                )}
              </tr>
      ...
      <div className="space-y-3 sm:hidden">
        {rows.map((row) => (
          <div
            key={row.id || JSON.stringify(row)}
            className="rounded-2xl border border-purple-100 bg-white p-4 shadow-sm shadow-purple-100/50 dark:border-gray-800 dark:bg-gray-900 dark:shadow-black/30"
          >
            <div className="divide-y divide-purple-100 dark:divide-gray-800">
              {columns.map((column) => {
                const value = row[column];
                ...
                return (
                  ...
                  >
                    <span className="text-xs font-semibold uppercase tracking-[0.1em] text-gray-400 dark:text-gray-500">
                      {column.toLowerCase() === 'active'
                        ? 'Status'
                        : column}
                    </span>

                    <span className="truncate text-right text-sm text-gray-700 dark:text-gray-300">
                      {column.toLowerCase() === 'active' ? (
                        value ? (
                          <span className="inline-flex cursor-default items-center rounded-full bg-green-50 px-3 py-1 text-xs font-semibold text-green-700 dark:bg-green-500/10 dark:text-green-400">
                            Active
            ...
            {editPath && isHR && (
              <button
                ...
                className="mt-3 flex w-full items-center justify-center gap-2 rounded-xl border border-purple-100 py-2 text-sm font-medium text-purple-600 transition hover:bg-purple-50 dark:border-gray-700 dark:text-purple-400 dark:hover:bg-purple-500/10"
                title={`Edit ${singularTitle}`}
              >
                <PencilIcon />
                Edit {singularTitle}
